...
export default function RuleControls({ ruleName, enabled }: Props) {
  ...
  async function postAction(action: string) {
    ...
    setError('');
    try {
      const res = await fetch(`/api/process/rules/${encodeURIComponent(ruleName)}`, {
        method: 'POST',
  ...
  async function handleDelete() {
    if (!confirm(`Delete rule "${ruleName}" and all its results?`)) return;
  ...
  return (
    <span className="inline-flex items-center gap-2">
      ...
      <Link
        href={`/process/rules/${encodeURIComponent(ruleName)}/edit`}
