...
# Make mcp/daas/ (daas_database) importable. fd_daas_mcp is already importable
# via the conftest (src/ on sys.path).
_DAAS_DIR = Path(__file__).resolve().parents[1] / "src" / "fd_daas_mcp" / "mcp" / "daas"
...
def test_default_never_inside_installed_package(monkeypatch):
    ...
    pkg = Path(D.__file__).resolve().parents[2]  # fd_daas_mcp/ package root
