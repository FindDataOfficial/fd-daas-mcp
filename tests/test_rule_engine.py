...
_DAAS_MCP = Path(__file__).resolve().parents[1] / "src" / "fd_daas_mcp" / "mcp" / "daas"
if str(_DAAS_MCP) not in sys.path:
    sys.path.insert(0, str(_DAAS_MCP))

from daas_database import get_database  # noqa: E402
from fd_daas_mcp.models import Entity  # noqa: E402
...
def test_position_rule_css_extracts_codes():
    pytest.importorskip("lxml", reason="position rule css/xpath needs the `position` extra")
    pytest.importorskip("cssselect", reason="position rule css/xpath needs the `position` extra")
    _seed()
    sess = get_database().get_session()
    rule = SimpleNamespace(
        rule_type="position",
