...
`.mcp.json` 把 `command` 指向 `fd-daas-mcp/bin/fd-daas-mcp-server`，一个自定位 POSIX shell 脚本，设置 `PYTHONPATH` 并 exec `.venv/bin/python -m fd_daas_mcp.server`。

```bash
fd-daas-mcp/bin/fd-daas-mcp-server             # 启动服务器
fd-daas-mcp/.venv/bin/python -m fd_daas_mcp.selfcheck   # 离线不变量
```
