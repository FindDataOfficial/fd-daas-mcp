...
A rule MAY carry a `datasource` name (daas `sources.name`) for traceability. The server SHALL NOT read from or write to any daas registry table (`sources`, `daas_functions`, `observations`, `datasource_*`). The source-of-truth for which table to read is `process_rules.source_table`; the daas `sources.config.scraw_config` slug is what makes the rule and the datasource point at the same scraped data.
