...
The `mcp/cnreport-mcp/server.py` FastMCP server SHALL register five additional tools — `get_company`, `list_filings`, `get_filing`, `get_financials`, `get_section` — alongside the existing PDF/AI/ES tools. The five new tools SHALL constitute the China-A-share counterpart of `edgartools-mcp`'s company surface. Existing tools (`list_outline`, `extract_section`, `ai_extract`, `index_records`, `search_reports`, `delete_index`) SHALL remain registered and behave unchanged.

#### Scenario: Server starts and registers all eleven tools

- **WHEN** the server is started with `uv run --directory mcp/cnreport-mcp python server.py`
- **THEN** the eleven tools — six existing plus the five new ones — are all registered and callable over stdio
...
The `list_filings(ticker_or_name, form=None, year=None, limit=20)` tool SHALL list disclosures for a company via CNINFO's `hisAnnouncement` endpoint, optionally filtered by form type (`年度报告`, `半年度报告`, `第一季度报告`, `第三季度报告`, etc.) and year. Each entry SHALL include `announcement_id`, `title`, `form`, `published`, `pdf_url`, and `stock_code`. Results SHALL be capped at `limit`.
