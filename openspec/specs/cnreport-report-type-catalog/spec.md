# cnreport-report-type-catalog Specification

## Purpose

Provide a data-driven JSON registry of CNINFO disclosure category codes for `mcp/cnreport-mcp/`, exposing them via a `list_report_types` MCP tool so callers can discover and address CNINFO report categories (periodic and special) without hardcoding. Replaces the previous hardcoded `_FORM_CATEGORIES` dict with an extensible `cninfo_categories.json` file, and lets `list_filings(category=…)` and `get_special_report(category=…)` resolve any catalog name or code without code changes.
