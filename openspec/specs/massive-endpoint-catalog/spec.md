# massive-endpoint-catalog Specification

## Purpose
Register Massive.com's per-endpoint REST API (~37 endpoints across 8 asset classes — Reference / Stocks / Options / Crypto / Forex / Futures / Indices / Economy / Alternative) as daas `daas_functions` + `daas_function_columns` under the existing `massive` source (created by `external-mcp-datasource-seed`), so agents discover Massive's data surface and its response columns via `search_functions` / `get_function_detail`. Adds `indicator_rules` (sma / ema / pct_change / zscore / rolling_std / level) over the Economy time-series endpoints (Treasury yields, inflation, inflation expectations, labor market) and a standalone `fastmcp.Client` backfill that populates `scraw_massive_*` tables so `run_indicator` computes real `observations`. Entitlement-gated endpoints (HTTP 403 on the current plan) are registered as metadata (`parameters.gated=true`) — discoverable but not backfilled, with no indicators.
## Requirements
