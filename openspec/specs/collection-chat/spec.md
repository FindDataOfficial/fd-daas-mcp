...
Per-collection chat surface in the dashboard. Each chat session is bound to exactly one `daas-mcp` collection; the active collection's items (datasources + sections + their `instruction` text) form the system context, and the model dispatches data requests via the dashboard's MCP-tools wiring (`fetch_data`, `list_sources`, ...). Streamed through a Next.js API route under `/api/collections/[name]/chat`.
