...
Persist structured row data returned by data-fetch function calls into a `data_snapshots` table in `daas.db`, so results can be reviewed later without re-fetching. Covers the storage schema, status semantics, and the dashboard view for browsing snapshot rows.
