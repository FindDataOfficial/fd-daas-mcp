# process-dashboard-ui Specification
...
#### Scenario: Rule detail renders without daas-mcp
- **WHEN** `daas-mcp` cannot start and the user visits `/process/rules/<existing-name>`
...
- **THEN** the dashboard calls `create_rule` via `daas-mcp` and the new rule appears on `/process/rules` after refresh
...
- **WHEN** `daas-mcp` cannot start when the create form renders
- **THEN** the form degrades to free-text inputs with an inline warning and remains submittable

### Requirement: Process mutations go through daas-mcp tools
The dashboard SHALL perform every rule/indicator mutation (create, update, delete, enable/disable toggle, run) by invoking the corresponding `daas-mcp` tool (`create_rule`, `update_rule`, `delete_rule`, `run_rule`, `create_indicator`, `update_indicator`, `delete_indicator`, `run_indicator`) through `getMCPTools()` from `dashboard/src/lib/mcp-client.ts`, exposed via `/api/process/*` Next.js routes. The dashboard SHALL NOT write to `process_rules`, `process_results`, `indicator_rules`, or `observations` via direct SQL. After a successful mutation, the API route SHALL call `invalidateDb('daas')` so the next render re-reads the file. When `daas-mcp` is unavailable, the route SHALL return a clear `{error}` to the client.

#### Scenario: Run-rule mutation routes through daas-mcp
- **WHEN** the user clicks "Run rule" on `/process/rules/<name>` and `daas-mcp` is available
- **THEN** the `/api/process/rules/<name>` route calls `run_rule` via `daas-mcp` through `getMCPTools()`, calls `invalidateDb('daas')`, and returns the result to the client

#### Scenario: daas-mcp unavailable returns a clear error
- **WHEN** `daas-mcp` cannot start and a mutation route is invoked
- **THEN** the route returns a `{error}` JSON response to the client without writing to `daas.db`
