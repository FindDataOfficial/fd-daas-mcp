# dashboard-registry Specification

## Purpose

A `dashboards` table in the shared `mcp/daas.db` database plus `dashboard-mcp` CRUD tools that back both dashboard skills. The DB is the single source of truth for dashboard metadata, and `dashboard/my-charts-dashboard/index.html` and `daas.md` are regenerated from it on every mutation.
