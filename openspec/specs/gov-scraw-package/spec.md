# gov-scraw-package Specification

## Purpose
A standalone, pip-installable Python package (`gov-scraw`) that bundles the 11 Chinese central-government ministry scrapers as a self-contained product decoupled from the `mcp/` monorepo. The package ships a crawl CLI, a bundled self-contained datasource registry (SQLite + JSON) derived from each script's `MANIFEST`, a read API for programmatic datasource/column discovery, an idempotent registry rebuild command, and GitHub-ready project metadata — enabling consumers to install and crawl without any MCP dependency.
