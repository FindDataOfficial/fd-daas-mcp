# datasource-forms-sections

## Purpose

A two-level structure under each datasource — `datasource_forms` (e.g. 10-K, 8-K) and `datasource_sections` (named sections with extraction instructions) — exposed via `add_form`, `add_section`, and `list_forms` tools.
