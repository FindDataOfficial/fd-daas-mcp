...
### Requirement: composite-mcp serves one selected composite

The system SHALL provide a FastMCP stdio server at `mcp/composite-mcp/server.py` that reads a composite name from the `COMPOSITE` environment variable at startup and serves that composite's curated tool surface.
...
The system SHALL register composite-management tools on every composite-mcp instance regardless of the active composite: `list_composites`, `create_composite`, `list_upstreams`, `add_upstream`, `remove_upstream`, `list_available_tools`, `add_tool`, `remove_tool`, `list_composite_tools`, `add_chained_tool`, `remove_chained_tool`, `list_chained_tools`.
