# trading-mcp-server Specification

## Purpose
TBD - created by syncing change add-trading-mcp. Update after archive.

Defines the FastMCP server at `mcp/trading-mcp/server.py` that exposes the trading-analysis tools (`analyze_ticker`, `bull_bear_debate`, `risk_debate`, `full_pipeline`, `list_personas`), self-registers them into the unified `daas.db` registry with `harness="trading"`, and declares the Pydantic output schemas for persona analyses and pipeline decisions.
