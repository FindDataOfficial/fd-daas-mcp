# fd-daas-dashboard Specification

## Purpose

The use skill that finds, opens, and inspects existing standalone HTML daas dashboards via the `dashboards` registry. It is read-only over the registry — building and editing are handled by `fd-daas-dashboard-creator`.
