# datasource-multi-level-search

## Purpose

A `search_datasources` tool that filters across category (with optional subtree), source, form, and section levels with an optional free-text query, expanding the form/section tree only when relevant to the drill level.
