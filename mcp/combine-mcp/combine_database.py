"""Database + upstream-client helpers for combine-mcp.
...
Usage:
    from combine_database import get_combine_db
    db = get_combine_db()
...
class CombineDatabase:
    ...
    def init_db(self) -> None:
        ...
        logger.info("Combine DB initialized: %s", self._database_url)
...
_combine_db: Optional[CombineDatabase] = None


def get_combine_db(database_url: Optional[str] = None) -> CombineDatabase:
    global _combine_db
    if _combine_db is None:
        _combine_db = CombineDatabase(database_url)
        _combine_db.init_db()
    return _combine_db


def reset_combine_db() -> None:
    global _combine_db
    if _combine_db is not None:
        _combine_db.dispose()
    _combine_db = None
