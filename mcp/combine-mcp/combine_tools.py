...
2. Served tools (built at startup) — proxied upstream tools (mounted via
   create_proxy + Visibility filter) and chained tools (built by
   `make_chain_tool`). These are wired in server.py.
"""
...
from combine_database import build_client, get_combine_db
...
def list_composites() -> str:
    ...
    rows = get_combine_db().list_composites()
    if not rows:
...
def create_composite(name: str, description: Optional[str] = None) -> str:
    ...
    """
    row = get_combine_db().create_composite(name, description)
...
def _require_composite(name: str):
    comp = get_combine_db().get_composite_by_name(name)
    if comp is None:
...
def list_upstreams(composite: str) -> str:
    ...
    rows = get_combine_db().list_upstreams(comp.id)
    if not rows:
...
def add_upstream(
    ...
    row = get_combine_db().add_upstream(
...
def remove_upstream(composite: str, key: str) -> str:
    ...
    comp = _require_composite(composite)
    get_combine_db().remove_upstream(comp.id, key)
...
async def list_available_tools(
    ...
    """
    comp = _require_composite(composite)
    upstreams = get_combine_db().list_upstreams(comp.id)
...
def add_tool(
    ...
    """
    comp = _require_composite(composite)
    get_combine_db().add_tool(comp.id, upstream_key, tool_name, alias)
...
def remove_tool(composite: str, upstream_key: str, tool_name: str) -> str:
    ...
    comp = _require_composite(composite)
    get_combine_db().remove_tool(comp.id, upstream_key, tool_name)
...
def list_composite_tools(composite: str) -> str:
    ...
    comp = _require_composite(composite)
    rows = get_combine_db().list_composite_tools(comp.id)
    if not rows:
...
def add_chained_tool(
    ...
    """
    comp = _require_composite(composite)
    get_combine_db().add_chain(comp.id, name, steps, description)
...
def remove_chained_tool(composite: str, name: str) -> str:
    ...
    comp = _require_composite(composite)
    get_combine_db().remove_chain(comp.id, name)
...
def list_chained_tools(composite: str) -> str:
    ...
    comp = _require_composite(composite)
    rows = get_combine_db().list_chains(comp.id)
    if not rows:
