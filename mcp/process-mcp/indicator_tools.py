"""process-mcp indicator tools — deterministic math indicators over a datasource's columns.

A fixed catalog of pandas one-liners (pct_change, sma, ema, rsi, ...). No LLM,
no `df.eval` of user input. Results upsert into the daas `observations` table
(via process_database.run_indicator); the ad-hoc `calculate` returns the series
without persisting.
