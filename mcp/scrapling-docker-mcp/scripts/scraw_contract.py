"""Stable contract for scraw scraper scripts.

Every scraper authored by the fd-daas-scrapling-scraw-creator skill ships a
module-level `MANIFEST = ScrawManifest(...)`. The manifest is the single
source of truth for the script's identity, crawl recipe, and output record
schema. `scripts/register.py` consumes it to write the daas database; the
dashboard reads the same blob from `sources.config`.

Stdlib only (dataclasses + json) — no extra dependency across MCPs.

Field naming: this module uses Python-idiomatic names (`name`, `type`,
`primary_key`, `nullable`). The `to_columns_json()` / `to_config_json()` /
`to_scraw_columns()` methods translate them to the on-disk shapes that
`datasource_columns` and `scraw_configs` actually use (`column_name`,
`column_type`, `is_primary_key` 0/1, etc.).

Self-check: `python3 scraw_contract.py` runs a round-trip assert.
"""
...
class ScrawManifest:
    ...
    `name` must equal the script's basename (without `.py`) — `register.py`
    uses it to look up the matching `sources` row and to label
    `datasource_columns.table_name`.
    """
    name: str
    label: str
    ...
    def to_config(self) -> dict:
        """Full self-describing blob stored in `sources.config` (dashboard reads this)."""
        return {
            "type": "scraw",
            "runtime": "uv",
            "scraper_script": f"mcp/scrapling-uv-mcp/scripts/{self.name}.py",
            "scraper_script_docker": f"mcp/scrapling-docker-mcp/scripts/{self.name}.py",
            "seed_url": self.url,
...
def _selfcheck() -> None:
    """Round-trip assert. Run with `python3 scraw_contract.py`."""
    m = ScrawManifest(
        name="demo_src",
        label="Demo",
    ...
    assert blob["type"] == "scraw" and blob["runtime"] == "uv", blob
    print("scraw_contract self-check OK")
