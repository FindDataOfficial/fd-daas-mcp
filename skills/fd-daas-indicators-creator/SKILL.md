...
Goal: create `scraw_<slug>` and upsert the fetched rows into it. The fetch itself goes through `fd-daas-based-data-fetch` (resolve via sqlite3 -> call the Python lib via `dispatch.py` -> persist via `upsert.py`).
...
- **`daas-mcp`'s `fetch_data` is gone** - the fetch goes through `fd-daas-based-data-fetch` calling the Python lib directly (see `dispatch.py`).
