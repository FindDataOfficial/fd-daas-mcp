...
- **`uv run` is broken for fd-daas-mcp** - always invoke the CLI via
  `fd-daas-mcp/.venv/bin/python -m fd_daas_mcp.cli ...`.
...
All run from the repo root with `fd-daas-mcp/.venv/bin/python -m fd_daas_mcp.cli`:

```bash
# Ingest a local PDF
fd-daas-mcp/.venv/bin/python -m fd_daas_mcp.cli pdf ingest_document file_path=/path/to/filing.pdf --json

# Ingest from a URL (edgar/cnreport/edinet filing URL)
fd-daas-mcp/.venv/bin/python -m fd_daas_mcp.cli pdf ingest_document url=https://www.sec.gov/.../10-K.pdf --json

# Ingest raw text
fd-daas-mcp/.venv/bin/python -m fd_daas_mcp.cli pdf ingest_text text="..." name=transcript --json

# Search
fd-daas-mcp/.venv/bin/python -m fd_daas_mcp.cli pdf search_documents query="revenue growth" top_k=5 --json

# Search within one document
fd-daas-mcp/.venv/bin/python -m fd_daas_mcp.cli pdf search_documents query="dividend policy" doc_id=3 --json

# List / get / delete
fd-daas-mcp/.venv/bin/python -m fd_daas_mcp.cli pdf list_documents --json
fd-daas-mcp/.venv/bin/python -m fd_daas_mcp.cli pdf get_document doc_id=3 --json
fd-daas-mcp/.venv/bin/python -m fd_daas_mcp.cli pdf delete_document doc_id=3 --json
```
...
fd-daas-mcp/.venv/bin/python -m fd_daas_mcp.cli --pdf-ingest /path/to/filing.pdf
fd-daas-mcp/.venv/bin/python -m fd_daas_mcp.cli --pdf-search "revenue growth"
```
