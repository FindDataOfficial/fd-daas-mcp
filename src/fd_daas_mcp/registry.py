...
Each source group's tool code lives in-package at ``src/fd_daas_mcp/mcp/<group>/``
(moved, not rewritten, from the former ``fd-daas-mcp/<group>-mcp/`` dirs). This
module imports each group's tool functions with per-group ``sys.modules``
isolation and returns ``[(group, tool_name, func)]`` for the server/CLI to
register under the collision-free ``<group>_<tool>`` namespace.
...
# Tool groups ship as package data under ``src/fd_daas_mcp/mcp/<group>/`` (and
# at the same relative path inside an installed wheel). Resolving from
# ``__file__`` keeps discovery correct across dev (editable install / src on
# PYTHONPATH) and installed (site-packages) shapes.
FD_HOME = Path(__file__).resolve().parent / "mcp"

SOURCES: dict[str, dict[str, Any]] = {
    "alerts":    {"dir": "alerts",    "inline": True},
    "cron":      {"dir": "cron",      "inline": True,  "suppress": True},
    "composite": {"dir": "composite", "inline": False},
    "daas":      {"dir": "daas",      "inline": False},
    "dashboard": {"dir": "dashboard", "inline": True},
    "leader":    {"dir": "leader",    "inline": False},
    ...
    "research":  {"dir": "research",  "inline": False},
    ...
    "pdf":       {"dir": "pdf",       "inline": False, "optional": True, "dep": "sqlite_vec"},
