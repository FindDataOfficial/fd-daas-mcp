...
def _sibling_tools() -> dict:
    ...
    Uses the consolidated-server package path ``fd_daas_mcp.registry``
    (the launch is ``python -m fd_daas_mcp.server`` with ``daas`` as the
    top package).
    """
    from fd_daas_mcp.registry import build, namespaced
