...
from fd_daas_mcp.models import (
    DaasFunction,
    DaasFunctionColumn,
    DaasSource,
    Category,
...
class EntityCollectionService:
    ...
    def sync_entity_collection(self, name: str) -> dict:
        ...
        intended_ids = set(RuleEngine.evaluate(rule_obj, self._session, db_url))
    ...
    def _rule_entity_ids(self, rule: dict) -> list[int]:
        """Apply a legacy declarative `rule_json` filter to `entities` and
        return matching entity ids. Delegates to the RuleEngine (json type).
        Kept for `entity_collection_sync.py --dry-run` and the selfchecks.
        """
