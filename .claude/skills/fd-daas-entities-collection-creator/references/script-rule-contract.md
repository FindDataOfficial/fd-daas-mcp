...
Any table in daas.db — `entities`, `observations`, `entity_datasource_links`, `entity_collection_items` (another collection's current members), and any `scraw_*` snapshot table (e.g. `scraw_stock_zh_a_hist`, `scraw_dzjy`). Use `sqlite3 mcp/daas.db ".tables"` to discover what's available, and `PRAGMA table_info(<table>)` for columns.
...
`mcp/daas-mcp/rules/entity_collections/<collection_name>.py` — co-located with daas-mcp, version-controlled (it's an authored rule, not generated data). The Write tool creates the parent dir. Pass the **repo-root-relative** path to `create_entity_collection(rule_script=…)`:

```
mcp/daas-mcp/rules/entity_collections/<collection_name>.py
```

## How to debug a script that matched zero entities

```bash
# 1. Dry-run the sync (reports rule_kind, rule_script path, intended_members):
uv run --directory mcp/daas-mcp python entity_collection_sync.py --sync <name> --dry-run

# 2. Run the script's own query against the DB to see what it would return:
sqlite3 mcp/daas.db "SELECT code FROM entities WHERE exchange='SSE'"

# 3. Edit the script, re-run the sync:
uv run --directory mcp/daas-mcp python server.py --sync-entity-collection <name>
```

`intended_members` is the count the script returned (after unknown-code skipping); `current_members` is what's in the collection now. If `intended_members` is null, the collection has no rule (manual).
