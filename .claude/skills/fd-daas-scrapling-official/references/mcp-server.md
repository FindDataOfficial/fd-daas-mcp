...
The Scrapling MCP server exposes ten fetch/session/screenshot tools over the MCP protocol, plus two reusable-script tools (`find_scripts`, `run_script`) added by this project. It supports CSS-selector-based content narrowing (reducing tokens by extracting only relevant elements before returning results), three levels of scraping capability (plain HTTP, browser-rendered, and stealth/anti-bot bypass), persistent browser session management, and page screenshots returned as real image content blocks.

**Hosting in this project:** the `scrapling` MCP group was **dropped** from the consolidated `fd-daas-mcp` server (only the `pdf` optional group survives). In this project, use the scrapling **library / CLI directly** as documented in `SKILL.md` (`pip install "scrapling[all]"` + `scrapling extract ...`). The MCP-server documentation below describes the **upstream `scrapling mcp` server** for standalone use only - it is not wired into `fd-daas-mcp`, so the `mcp__fd-daas-mcp__scrapling_*` / `fd-daas-mcp scrapling` invocations no longer exist. The former `fd-daas-mcp/scrapling-mcp/` package was removed.
...
The project-added `find_scripts` / `run_script` tools that previously lived in `fd-daas-mcp/scrapling-mcp/scripts/` were removed with the scrapling group - they are unavailable in this project.
...
**This project:** the scrapling MCP group is not hosted by `fd-daas-mcp` (dropped). Use the library/CLI directly (`pip install "scrapling[all]"` + `scrapling install` for browser binaries) as shown in `SKILL.md`. The standalone `scrapling mcp` setup below applies only if you choose to run the upstream MCP server yourself.
