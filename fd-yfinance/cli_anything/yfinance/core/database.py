"""
Database module for yfinance registry.

Provides SQLAlchemy engine and session management. Reads YFINANCE_DATABASE_URL
...
from cli_anything.yfinance.core.models import Base
...
class Database:
    ...
    Reads YFINANCE_DATABASE_URL env var; defaults to SQLite at metadata/registry.db.
    Singleton — use get_database() instead of constructing directly.
    """

    def __init__(self, database_url: Optional[str] = None):
        if database_url is None:
            # empty string env var == unset → fall back to default
            database_url = os.environ.get("YFINANCE_DATABASE_URL") or (
